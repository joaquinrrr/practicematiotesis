...
const MathQuiz: React.FC = () => {
  ...
  useEffect(() => {
    // Fetch 5 random "resta" problems from Supabase
    const fetchProblems = async () => {
      setLoading(true);
      const { data, error } = await supabase
        .from('problemas') // Nombre de la tabla
        .select('*')
        .eq('tipo_operacion', 'resta') // Filtrar por tipo_operacion = resta
  ...
  if (problems.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white">
        No se encontraron problemas de resta.
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen w-full bg-gray-900 p-6 text-white flex flex-col items-center justify-center">
      <h1 className="text-4xl font-bold mb-4">Problemas de Resta</h1>
      <p className="text-lg text-gray-300 mb-6">
        Responde las siguientes restas correctamente.
      </p>
